...
인스타그램 릴스에 올리는 것과 같은 쇼츠 영상을 매일 본인 유튜브 채널에도
자동 업로드합니다. 제목·설명·해시태그는 그날의 공고 내용으로 자동 생성됩니다.
(예: "8월 3일 마감임박 공공기관 채용 TOP5 | 한국전력공사 외 #Shorts")

### 설정 (1회, 약 2분)

유튜브 업로드는 API 키로는 불가능하고(조회 전용) 채널 소유자 OAuth 인증이
필요합니다. 드라이브 인증 때와 같은 방식입니다:

```bash
cd ~/puconet/automation && python3 youtube_auth.py
```

화면의 URL 접속 → 코드 입력 → 유튜브 채널이 있는 구글 계정으로 로그인 → 허용.
"확인하지 않은 앱" 경고가 나오면 '고급' → '이동'을 클릭하면 됩니다.
(드라이브 때 만든 OAuth 클라이언트를 자동 재사용하므로 추가 준비물 없음)

### 선택 설정 (`_config/shorts` 문서)

| 필드 | 기본값 | 설명 |
|---|---|---|
| `youtube` | `true` | `false` 로 두면 유튜브 업로드만 중지 |
| `youtube_privacy` | `public` | `unlisted`(일부공개)/`private` 로 변경 가능 |
| `youtube_category` | `22` | 유튜브 카테고리 ID |
...
언론사 사진 대신 **저작권 걱정이 없는 카드 이미지를 자동 생성**해 붙입니다:
① 기사에 등장하는 공공기관의 CI 로고(Storage `logos/` 보유분)를 찾으면 → 기관 CI 카드,
② 없으면 → 기사 제목을 넣은 브랜드 색상 카드. 생성된 이미지는 Storage
`news_images/auto/` 에 저장됩니다. 원문 기사 사진을 꼭 쓰려면 `use_og_image` 를
`true` 로 바꾸면 되지만, 한국 언론사 사진은 출처를 표기해도 무단 사용에 해당해
손해배상 청구 사례가 많으므로 권장하지 않습니다.
